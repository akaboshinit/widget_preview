...
// TODO(bkonyi): update this documentation

/// A testing stub for the system's onscreen keyboard.
///
/// Typical app tests will not need to use this class directly.
///
/// The [TestWidgetsFlutterBinding] class registers a [PreviewTextInput] instance
...
/// [closeConnection] methods can be used even when the [PreviewTextInput] is not
...
class PreviewTextInput {
  ...
  void register() =>
      WidgetsBinding.instance.defaultBinaryMessenger.setMessageHandler(
        ...
        _handler(_handleTextInputCall),
      );

  MessageHandler _handler(Future<dynamic> Function(MethodCall) callback) {
    return (ByteData? message) async {
  ...
  /// Whether this [PreviewTextInput] is registered with [SystemChannels.textInput].
  ///
  ...
  /// [WidgetTester.pump].
  ///
  /// This can be called even if the [PreviewTextInput] has not been [register]ed.
  ///
  ...
  Future<void> finishScribbleInteraction() async {
    ...
    ServicesBinding.instance.channelBuffers.push(
      SystemChannels.textInput.name,
      SystemChannels.textInput.codec.encodeMethodCall(
        MethodCall('TextInputClient.scribbleInteractionFinished', <dynamic>[
          _client ?? -1,
        ]),
      ),
      (ByteData? data) {/* response from framework is discarded */},
  ...
  Future<void> scribbleFocusElement(
      String elementIdentifier, Offset offset) async {
    assert(isRegistered);
    ServicesBinding.instance.channelBuffers.push(
      SystemChannels.textInput.name,
      SystemChannels.textInput.codec.encodeMethodCall(
        MethodCall('TextInputClient.focusElement',
  ...
  Future<List<List<dynamic>>> scribbleRequestElementsInRect(Rect rect) async {
    ...
    ServicesBinding.instance.channelBuffers.push(
      SystemChannels.textInput.name,
      SystemChannels.textInput.codec.encodeMethodCall(
        MethodCall('TextInputClient.requestElementsInRect',
            <dynamic>[rect.left, rect.top, rect.width, rect.height]),
      ),
      (ByteData? data) {
        response = (SystemChannels.textInput.codec.decodeEnvelope(data!)
                as List<dynamic>)
  ...
  Future<void> scribbleInsertPlaceholder() async {
    ...
    ServicesBinding.instance.channelBuffers.push(
      SystemChannels.textInput.name,
      SystemChannels.textInput.codec.encodeMethodCall(
        MethodCall('TextInputClient.insertTextPlaceholder',
  ...
  Future<void> scribbleRemovePlaceholder() async {
    ...
    ServicesBinding.instance.channelBuffers.push(
      ...
      SystemChannels.textInput.codec.encodeMethodCall(
        MethodCall(
            'TextInputClient.removeTextPlaceholder', <dynamic>[_client ?? -1]),
      ),
      (ByteData? data) {/* response from framework is discarded */},
    );
  }
